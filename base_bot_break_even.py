...
Base Trading Bot v7
التغييرات:
  - إزالة stop_loss كاملاً
  - إصلاح C-03: run() يُمرر BUY فقط لـ _execute (SELL = لا تفعل شيئاً)
  - إصلاح H-02: _execute يستخدم sig.quantity بدلاً من إعادة الحساب
  - إصلاح M-05: _load_existing_positions تحسب take_profit صحيح
  - إصلاح L-01: _place_close_order تُرسل تنبيه عند الفشل وتُحاول مرة ثانية
"""
import time
import logging
import signal
...
def _setup_logging(bot_name: str, log_file: str):
    root = logging.getLogger()
    if root.handlers:
        return   # لا تُعيد التهيئة إذا كانت قد هُيِّئت
    logging.basicConfig(
        level=logging.INFO,
...
class BaseMetalBot:
    ...
    SCALP_ENABLED                    = True
    SCALP_MIN_HOURLY_VOLATILITY_PCT  = 0.008
    SCALP_ENTRY_DROP_PCT             = 0.005
    SCALP_TAKE_PROFIT_PCT            = 0.012
    SCALP_STOP_LOSS_PCT              = 0.008
    SCALP_TRAILING_STOP_PCT          = 0.005
    SCALP_MAX_SPREAD_TO_TP_RATIO     = 0.40
    SCALP_MAX_TRADES_PER_DAY         = 3
    SCALP_RSI_BUY_LEVEL              = 35.0
    SCALP_RSI_SELL_LEVEL             = 70.0
    ...
    def _fetch_balance(self):
        ...
        return parse_balance(root) if root is not None else {}
    ...
    def _load_existing_positions(self):
        ...
        # لا يمكن معرفة سعر الدخول الحقيقي من الرصيد فقط؛ نستخدم السعر الحالي تحفظياً وننبه المستخدم.
        take_profit  = self.risk.calculate_take_profit("B", current_price)
        stop_loss    = self.risk.calculate_stop_loss("B", current_price)
        # تحقق من state file مباشرة قبل الإضافة
        import json, os
        state_file = f"state_{self.SECURITY_ID}.json"
        if os.path.exists(state_file):
            try:
                with open(state_file) as _sf:
                    _st = json.load(_sf)
                if _st.get("open_trades"):
                    self.logger.info(f"state file يحتوي مراكز حقيقية — تخطي _load_existing_positions")
                    return
            except Exception:
                pass
        synthetic_id = f"VAULT_{self.SECURITY_ID}_loaded"
    ...
    def _monitor_positions(self, market_data, balance: dict):
        usd = balance.get("USD", {}).get("available", 0)
        for order_id, trade in list(self.risk.open_trades.items()):
            ...
            if hit_stop or hit_fixed_tp or hit_trailing:
                ...
                if (self.DCA_ENABLED and trade.dca_count < self.DCA_MAX_TIMES
                    ...
                    usd = self._fetch_balance().get("USD", {}).get("available", 0)
                ...
                self.logger.info(
                    ...
                    f"({change_pct:+.2f}%) | PnL=${pnl:.2f} | TP={trade.take_profit:,.0f}"
                    f"{trail_info}"
                )
    ...
    def _execute_dca(self, trade: TradeRecord, current_price: float,
        ...
        if orders:
            ...
            new_sl            = self.risk.calculate_stop_loss("B", avg_price)
    ...
    def _execute(self, sig, balance):
        ...
        resp = self.api.place_order(
            action="B", security_id=sig.security_id,
            currency=sig.currency, quantity=quantity,
            limit=limit_price, type_code="TIL_CANCEL",
        )
        if resp is None:
            return
        ...
        if orders:
            ...
            if status in {"DONE", "MATCHED", "PARTIALLY_MATCHED"}:
                ...
                sl = self.risk.calculate_stop_loss("B", limit_price)
                self.risk.register_trade(TradeRecord(
                    order_id=o["orderId"], action="B",
                    security_id=sig.security_id, currency=sig.currency,
                    quantity=actual_qty, entry_price=limit_price,
